    ...
    return Consumer<ProductDetailsProvider>(
      builder: (context, details, child) {
        ...
        return details.hasConnection ? Scaffold(
          ...
          bottomNavigationBar: BottomCartView(product: widget.banner? Provider.of<BannerProvider>(context, listen: false).product: widget.product),
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                widget.banner && Provider.of<BannerProvider>(context, listen: false).product != null?
                Consumer<BannerProvider>(
                  builder: (context, bannerP,_) {
                    print('====>XYZ======>${bannerP.product}');
                    return bannerP.product != null? Column(children: [
                      ProductImageView(productModel: bannerP.product),
                      ProductTitleView(productModel: bannerP.product)
            ],):SizedBox();
                  }
                ):
                Column(children: [
                ...
                widget.banner && Provider.of<BannerProvider>(context, listen: false).product != null?
                Provider.of<BannerProvider>(context, listen: false).product != null && Provider.of<BannerProvider>(context, listen: false).product.addedBy == 'seller' ? SellerView(sellerId: Provider.of<BannerProvider>(context, listen: false).product.userId.toString()) : SizedBox.shrink():
                widget.product.addedBy == 'seller' ? SellerView(sellerId: widget.product.userId.toString()) : SizedBox.shrink(),


                // Specification
                widget.banner && Provider.of<BannerProvider>(context, listen: false).product != null?
                (Provider.of<BannerProvider>(context, listen: false).product != null ) ? Container(
                  margin: EdgeInsets.only(top: Dimensions.PADDING_SIZE_SMALL),
                  padding: EdgeInsets.all(Dimensions.PADDING_SIZE_SMALL),
                  color: Theme.of(context).highlightColor,
                  child: ProductSpecification(productSpecification: Provider.of<BannerProvider>(context, listen: false).product.details ?? ''),
                ) : SizedBox():
